...
def main(paths, n, file_name, q):
    ...
    # make sure the all the loaded label maps have the same native size
    native_sizes = []
    ...
    # make sure all the native sizes are the same
    ...
    save_labelmap(z_cords, interpolators, native_sizes[0], affine, save_path="D:/DTUTeams/wall_segmentation2022/specialkursus/data/{}/interpolations2/{}.seg.nrrd".format(q, file_name))
...
if __name__ == "__main__":
    ...
    for q in ['train', 'val', 'test']:
        ...
        root = r"D:\DTUTeams\wall_segmentation2022\specialkursus\data\{}\labels".format(q)
        img_paths = r"D:\DTUTeams\wall_segmentation2022\specialkursus\data\{}\imgs".format(q)
        txt_path = r"D:\DTUTeams\wall_segmentation2022\specialkursus\data\{}".format(q)

        for i,f in enumerate(tqdm(sorted(os.listdir(img_paths)))):
            file_name = os.path.join(f.split(".")[0])

            with open(os.path.join(txt_path, 'interp.txt')) as g:
                s = g.read().splitlines()
                interp_parameters = [tuple(g.split(": ")[-1].split(",")) for g in s]
            a,b,c = interp_parameters[i]

            if a == 'True':
                paths = [os.path.join(root, file_name+".seg.nrrd")] + [os.path.join(root, file_name+"%{}.seg.nrrd".format(i)) for i in range(int(b),int(c))]
            elif a == 'False':
                paths = [os.path.join(root, file_name + "%{}.seg.nrrd".format(i)) for i in range(int(b),int(c))]
